...
class Test(TestCase):
    ...
    def test_raise(self):
        ...
        self.assertRaises(
            EasyProcessError, lambda: EasyProcess('ls -la').stop())
        self.assertRaises(
            EasyProcessError, lambda: EasyProcess('ls -la').sendstop())
        # self.assertRaises(EasyProcessError, lambda : EasyProcess('ls
        # -la').start().stop().stop())
        self.assertRaises(EasyProcessError, EasyProcess('ls -la')
                          .start().wrap(lambda: None))
